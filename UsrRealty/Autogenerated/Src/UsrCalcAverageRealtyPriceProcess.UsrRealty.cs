namespace Terrasoft.Core.Process
{
	...
	public class UsrCalcAverageRealtyPriceProcessMethodsWrapper : ProcessModel
	{
		...
		private bool ScriptTask1Execute(ProcessExecutingContext context) {
			...
			var priceColumn = esq.AddColumn("UsrPrice"); // SELECT UsrPrice as UsrPrice, UsrArea as
			...
			esq.Filters.Add(offerTypeFilter); // ... WHERE UsrTypeId = :typeId AND UsrOfferTypeId =
			...
			foreach(var entity in entityCollection) {
			    decimal price = entity.GetTypedColumnValue<decimal>(priceColumn.Name); // reading u
			    decimal area = entity.GetTypedColumnValue<decimal>(areaColumn.Name); // reading usi
